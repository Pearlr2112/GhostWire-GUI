...
def open_jitsi_window(url: str, title: str = "GhostWire Call") -> None:
    try:
        import webview, subprocess, sys
        script = (
            "import webview\n"
            "w = webview.create_window(" + repr(title) + ", " + repr(url) + ", width=1024, height=720, resizable=True)\n"
            "webview.start()\n"
        )
...
def open_jitsi_window(url: str, title: str = "GhostWire Call") -> None:
    ...
    pywebview MUST run on the main thread on macOS (WKWebView requirement).
    We use a subprocess so it gets its own main thread, completely separate
    from the tkinter main loop.

    Falls back to the system browser if pywebview isn't installed.
    """
    try:
        import webview  # just check it's installed; raises ImportError if not

        # Launch a fresh Python process — it gets a clean main thread for pywebview
